...
def structural_checks():
    ...
    assert model['canon_id'] == 'ASET-WORKER-CANON-0.1-ALPHA2'
    assert model['version'] == '0.1.0-alpha.1'
    assert model['state_machine']['terminal_states'] == ['RESULT','NO_RESULT']
    assert model['state_machine']['terminal_relation'] == 'XOR'
    assert model['state_machine']['stored_or_derived_states'] == ['RUNNING','RESULT','NO_RESULT']
    assert [x['kind'] for x in model['operations']] == ['START_WORK','END_WORK']
    assert len(model['requirements']) == 9
    assert len(model['invariants']) == 9
    ...
    assert 'ACCEPT_WORK' not in text and 'ACCEPTED' not in text
